...
class ReadHandler<T extends Client<Connection<T>>> implements CompletionHandler<Integer, T> {
    ...
    private void readData(T client) {
        ...
        if (buffer.remaining() < Client.HEADER_SIZE){
        ...
        int dataSize = Short.toUnsignedInt(buffer.getShort()) - Client.HEADER_SIZE;

        if(dataSize > buffer.remaining()) {
            LOGGER.debug("Not enough data to read. Packet size {}", dataSize);
            buffer.position(buffer.position() - Client.HEADER_SIZE);
            buffer.compact();
            connection.read();
            return;
        }
        ...
        try {
            if (dataSize > 0) {
                ...
                if (!buffer.hasRemaining()) {
                    ...
                    buffer.compact();
                    if (remaining >= Client.HEADER_SIZE) {
